...
import {
    ...
    storage,
    SUCCESS_TASKS,
    type DownloadSuccessMessage
} from "~lib/helper";

const handler: PlasmoMessaging.MessageHandler<DownloadSuccessMessage> = async (
    req,
    res
) => {
    try {
        const [prevInProgressValue, prevSuccessValue, count] =
            await Promise.all([
                storage.get(IN_PROGRESS_TASKS) as Promise<
                    Array<DownloadInProgressMessage>
                >,
                storage.get(SUCCESS_TASKS) as Promise<
                    Array<DownloadSuccessMessage>
                >,
        ...
        const inProgressIndex = prevInProgressValueRes.findIndex(
            (task) => task.url === url
        );
